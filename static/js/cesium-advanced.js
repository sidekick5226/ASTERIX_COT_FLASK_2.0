...
class AdvancedCesiumManager {
    ...
    startFollowCamera(entity) {
        // No confirm dialog in Battle View - directly start following
        this.followTarget = entity;
        this.viewer.trackedEntity = entity;

        // Set optimal follow distance based on track type
        const trackType = entity.properties?.trackType?.getValue();
        let followDistance = 10000; // Default 10km

        if (trackType === 'Aircraft') {
            followDistance = 20000; // 20km for aircraft
        } else if (trackType === 'Vessel') {
            followDistance = 5000; // 5km for vessels
        } else if (trackType === 'Vehicle') {
            followDistance = 2000; // 2km for ground vehicles
        }

        // Set camera to follow at optimal distance
        this.viewer.camera.setView({
            destination: entity.position.getValue(this.viewer.clock.currentTime),
            orientation: {
                heading: 0,
        ...
        console.log(`Following ${entity.name || entity.id} at ${followDistance}m distance`);
    }
